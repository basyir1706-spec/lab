...
| **#17** | Selaraskan gaya kad metrik Digunakan dengan meletakkan latar belakang kuning/amber terang di depan dan ikon jam cerah/putih seragam seperti kad metrik yang lain | Mengemaskini `index.html` dan `SOFTWARE_DESIGN.md` | ✅ Selesai |
| **#18** | Optimumkan responsif paparan peranti mudah alih (*mobile*) dengan menyembunyikan sidebar besar yang mengganggu skrin, membina *mobile top header bar* melekit (sticky) dengan butang hamburger, menu laci *off-canvas drawer* berserta pengesanan aktif automatik, serta kawalan limpahan (*overflow*) pada jadual dan modal | Mengemaskini `index.html`, `script.js`, dan `SOFTWARE_DESIGN.md` | ✅ Selesai |
| **#19** | Buang teks label "Date Picker Strip" pada panel kanan dashboard agar paparan lebih bersih dan kemas khususnya pada skrin mudah alih (*mobile*), serta tetapkan `flex-shrink: 0` pada butang jalur tarikh | Mengemaskini `index.html`, `style.css`, dan `SOFTWARE_DESIGN.md` | ✅ Selesai |
| **#20** | Pindahkan bahagian jalur tarikh (Date Picker Strip) terus ke atas jadual Rekod Paparan Penggunaan Laboratorium; baiki dan pulihkan interaksi hover pada senarai makmal dengan efek lif mikro (*transform: translateY(-2px)*), cahaya oren berkilau, dan auto-kembang butiran makmal yang responsif (220ms) | Mengemaskini `index.html`, `style.css`, `script.js`, dan `SOFTWARE_DESIGN.md` | ✅ Selesai |
| **#21** | Ubah susunan kedudukan hanya untuk mod responsif mobile (skrin < xl) di mana bahagian div tarikh dan senarai makmal diletakkan di atas jadual penggunaan makmal (`order-2`), manakala jadual berada di bawah (`order-3`); paparan versi web desktop (`xl:`) dikekalkan 100% seperti asal (2 lajur: 7 cols kiri [4 kad + jadual], 5 cols kanan [tarikh, aras, senarai makmal]) dengan memanfaatkan utiliti `contents xl:block` dan flexbox ordering | Mengemaskini `index.html` dan `SOFTWARE_DESIGN.md` | ✅ Selesai |
